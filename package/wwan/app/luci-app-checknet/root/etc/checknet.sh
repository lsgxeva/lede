...
create_checknet_config()
{
	...
	uci -q batch <<-EOF >/dev/null
		...
		set checknet.@service[-1].minute='10'
		set checknet.@service[-1].log_enabled='0'
		set checknet.@service[-1].board_name='gl-ar300m16'
		set checknet.@service[-1].public_dnsip='114.114.114.114'
		set checknet.@service[-1].wan_count='3'
		...
		set checknet.@service[-1].ttyusb_count='9'
...
get_checknet_config()
{
	...
	PUBLIC_DNSIP="$( uci get checknet.@service[-1].public_dnsip )"
	WAN_COUNT=$( uci get checknet.@service[-1].wan_count )
	...
	TTYUSB_COUNT=$( uci get checknet.@service[-1].ttyusb_count )
...
check_net_status()
{
	local zone phy ifip gateway count
	if [ $# -ne 1 ]; then
		return 0
	fi
	zone=$1
	...
	logg "* check_net_status - zone:${zone}, phy:${phy}, IP:${ifip}, GW:${gateway} "
	count=$( ping -4 -I $phy -c 1 -W 2 -s 56 -t 60 -q $PUBLIC_DNSIP | grep -i ", 0% packet loss"  | wc -l )
	if [ $count -ne 0 ]; then
		logg "* check_net_status - ping check to $PUBLIC_DNSIP is OK"
		return 0
	else
		logg "* check_net_status - ping check to $PUBLIC_DNSIP is NG"
		return 1
	fi
}
...
check_mwan3_status()
{
	...
	zone=$1
	[ "$( mwan3 interfaces | grep ${zone} | awk '{print $8}' )" != "active" ] && 
		logg "* check_mwan3_status - $zone not up, please execute mwan3 ifup $zone" && return 1
	if [ "$( mwan3 interfaces | grep ${zone} | awk '{print $4}' )" != "online" ]; then
...
get_signal_strength()
{
	...
	[ $lte_asu -lt 0 -o $lte_asu -gt 99 ] && return 1
