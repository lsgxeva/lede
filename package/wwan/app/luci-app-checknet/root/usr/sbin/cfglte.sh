...
if [ -d "/sys/class/net/wwan0" -a -d "/sys/class/net/wwan1" -a $( uci get network.lan.ifname | wc -w ) -eq 1 ]; then
	if [ "$( uci get network.lte1 2>/dev/null )" == "interface" -a "$( uci get network.lte2 2>/dev/null )" == "interface" ]; then
		echo "info: not need config network lte, will exit."
		exit 0
	fi
	...
	if [ -f "/etc/config/checknet" ]; then
		uci -q batch <<-EOF >/dev/null
			...
			set checknet.@service[0].minute='1'
			commit checknet
		EOF
	fi
